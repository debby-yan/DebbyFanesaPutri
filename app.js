...
app.post(
  ...
  [
    body('nisn')
      .isLength({ min: 8, max: 8 }).withMessage('NISN harus 8 digit angka!')
      .custom(async (value) => {
    ...
    body('nik')
      .isLength({ min: 16, max: 16 }).withMessage('NIK harus 16 digit angka!')
      .custom(async (value) => {
...
app.put(
  ...
  [
    ...
    // VALIDASI NOKK (BIAR BISA DI EDIT)
    body('nokk')
      .isLength({ min: 16, max: 16 }).withMessage('No. KK harus 16 digit angka!')
    ...
    await Siswa.updateOne(
      { nisn: req.body.oldNisn }, 
      {
        $set: {
          nisn: req.body.nisn,
          nik: req.body.nik,
          nokk: req.body.nokk,          // <--- DITAMBAHKAN
